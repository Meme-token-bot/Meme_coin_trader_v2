"""
THE HISTORIAN V2 - With Multi-Webhook Auto-Discovery
Trading System V2

NOW INCLUDES:
- Real wallet discovery from token transactions
- Multi-webhook auto-management (scales past 25 wallets)
- API budget tracking
...
from hybrid_discovery import HybridDiscoverySystem
...
class Historian:
    """THE HISTORIAN - With multi-webhook auto-discovery"""
    
    def __init__(self, db, helius_key: str, discovery_key: str = None, multi_webhook_manager=None):
        self.db = db
        
        # Main scanner for monitoring (uses main API key)
        self.scanner = TokenScanner(db, helius_key)
        
        # Discovery scanner (uses dedicated key if provided)
        discovery_api_key = discovery_key or helius_key
        self.discovery_scanner = TokenScanner(db, discovery_api_key)
        ...
        self.hybrid_discovery = HybridDiscoverySystem(
            db=self.db,
        ...
        # Multi-webhook manager for auto-adding wallets (scales past 25)
        self.multi_webhook_manager = multi_webhook_manager
        if multi_webhook_manager:
            print("  ✅ Multi-Webhook auto-add enabled (unlimited scaling)")
        else:
            print("  ⚠️  Webhook auto-add disabled")
        
        print("\nðŸ“š THE HISTORIAN initialized")
    
    def run_discovery(self, notify_callback=None, max_wallets: int = None, api_budget: int = None) -> Dict:
        """Run hybrid discovery with multi-webhook integration"""
        ...
        stats = self.hybrid_discovery.run_discovery(
            api_budget=api_budget,
            max_wallets=max_wallets
        )
        
        # Get newly added wallets and auto-add to webhook system
        if stats.get('wallets_verified', 0) > 0:
            with self.db.connection() as conn:
                ...
                for row in rows:
                    ...
                    new_addresses.append(wallet_dict['address'])
                    
                    # Send notification
                    if notify_callback:
                        notify_callback('discovery', wallet_dict['address'], wallet_dict)
                
                # Auto-add to webhook system (handles multiple webhooks automatically)
                if self.multi_webhook_manager and new_addresses:
                    ...
                    # Add webhook stats to return value
                    ...
                    if webhook_stats['added'] > 0:
                        print(f"   ✅ {webhook_stats['added']} wallet(s) added to webhook system!")
                        
                        # Show updated webhook status
                        status = self.multi_webhook_manager.get_status()
                        print(f"   📊 System: {status['active_webhooks']} webhook(s), {status['total_wallets_assigned']} total wallets ({status['utilization_pct']:.0f}% capacity)")
                    
                    if webhook_stats['failed'] > 0:
                        print(f"   ⚠️  {webhook_stats['failed']} wallet(s) failed - run sync to retry")
                else:
                    stats['wallets_added_to_webhook'] = 0
                    stats['wallets_failed_webhook'] = 0
    ...
    def clear_caches(self):
        ...
        self.discovery_scanner.clear_cache()
